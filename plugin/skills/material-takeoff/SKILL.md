...
1. Detect the host platform and pick the right binary under
   `${CLAUDE_PLUGIN_ROOT}/skills/material-takeoff/bin/`:

- `darwin-arm64/mto` (Apple Silicon)
- `darwin-x64/mto` (Intel Mac)
- `win-x64/mto.exe`
...
5. If a binary for the host platform is missing, tell the user the plugin
   doesn't support this platform yet — do NOT fall back to another tool.
